...
| Existen el pasar del tiempo y las leyes fisicas fundamentales | **Parcial** | `sim/clock.ts` — tiempo y ciclo dia/noche; `sim/relief.ts` — el mundo tiene altura como campo continuo; `sim/systems/jump.ts` — gravedad, salto y caida | «el ciclo del dia es periodico», «el dia recorre sus cuatro fases», «la altura del suelo es continua al cruzar a un talud», «no se cambia de nivel andando», «salir de un borde describe un arco», «el agua es muro tambien en el aire» — la altura ya estorba; falta el resto de la fisica (empuje entre cuerpos, fluidos) |
