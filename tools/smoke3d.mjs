 ...
 * lo juega leyendo el estado real por `window.__spike` y guarda capturas.
 *
 ...
 *
 *   npm run spike && node tools/smoke3d.mjs
 *   VERDANT_URL=https://.../3d node tools/smoke3d.mjs   # verifica un despliegue
 *
...
const PAGE = fileURLToPath(new URL('../packages/client/dist-spike/spike3d.html', import.meta.url));
...
function state(page) {
  return page.evaluate(() => {
    const { spots, ...rest } = window.__spike;
    return rest;
  });
}
...
function spots(page) {
  return page.evaluate(() => window.__spike.spots());
}
...
async function waitForLoop(page, ticks = 90) {
  ...
  await page.waitForFunction(
    (n) => {
      if (!window.__spike) return false;
      if (window.__smokeBaseTick === undefined) {
        window.__smokeBaseTick = window.__spike.tick;
        return false;
      }
      return window.__spike.tick - window.__smokeBaseTick > n;
    },
    ticks,
    { timeout: 30000 },
  );
...
console.log('');
if (failures.length) {
  console.log(`HUMO 3D: FALLIDO (${failures.length})`);
  process.exitCode = 1;
} else {
  console.log('HUMO 3D: OK — sin errores de consola ni excepciones');
}
